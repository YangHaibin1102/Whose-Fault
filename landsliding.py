...
inputPath = os.path.join(os.path.abspath("."),"landsliding_randtest/")
if uw.rank() == 0:
...
velocityBCs = uw.conditions.DirichletCondition( variable        = velocityField, 
                                                indexSetsPerDof = (left+base, base) )
...
if(LoadFromFile == False): 
    ...
    for index,item in enumerate(sq):        
        plasticStrain.data[item] = 2
    
    '''
    core_number = 200
    circle_core = np.zeros((core_number,2))
    np.random.seed(0)
    circle_core[:,0] = np.random.uniform(minX, maxX-bufferLayer3,core_number)
    circle_core[:,1] = np.random.uniform(minY, maxY-4*bufferLayer2,core_number)
    
    def circle(x,y,x0,y0):
        return (x-x0)*(x-x0)+(y-y0)*(y-y0)
    
    dx = 1*min((maxX-minX)/resX,(maxY-minY)/resY)
    i = 0
    ...
    faultShape1  = np.array([ (0- dx,minY), (0.+ dx,minY),(0.+ dx,maxY-1*dx),(0.- dx,maxY-1*dx)])
    ...
    for index in range( len(swarm.particleCoordinates.data) ):
        ...
        if (fault1.evaluate(tuple(coord))):
            plasticStrain.data[index] = 1.5
    '''  
    coord = fn.input()
    conditions = [((coord[0] > maxX-bufferLayer3), materialB),
                  ((coord[1] > maxY-bufferLayer2), materialB),
                  (       True ,           materialA ) ]
...
if (Elasticity == True):
    mappingDictViscosity1   = { materialA: viscosityA,
                                materialB: nd(1e18*u.pascal*u.second)}
    ...
    dt_e    = nd(300*u.year)                        # elastic time step
...
while step<nsteps:
    ...
    if (step%5==0):  
        ...
        plasticStrain0.save(outputPath+"plasticStrain0"+str(step).zfill(4)) 
