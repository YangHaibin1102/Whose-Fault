...
outputPath = os.path.join(os.path.abspath("."),"fault_trace/")
inputPath = os.path.join(os.path.abspath("."),"fault_trace/")
if uw.rank() == 0:
...
minX  = nd(-0.5e2 * u.kilometer)
maxX  = nd(0.5e2 * u.kilometer)
minY  = 0.
maxY  = nd(2e2 * u.kilometer)
...
Vy = nd(4.*u.centimeter/u.year)
bufferLayer1 = 0.00001
bufferLayer2 = 0.00001
bufferLayer3 = 0.00001
...
if(LoadFromFile == False): 
    ...
    faultShape1  = np.array([ (0.- dx,0.-dx), (0.+ dx,0.-dx),(0.+dx,0.3*maxY+dx),(0.-dx,0.3*maxY+dx)])
    fault1 = fn.shape.Polygon( faultShape1 )

    faultShape2  = np.array([ (-0.2 - dx,0.3*maxY), (-0.2+dx,0.3*maxY),(-0.2+dx,0.6*maxY),(-0.2-dx,0.6*maxY)])
    fault2 = fn.shape.Polygon( faultShape2 )
    
    faultShape3  = np.array([ (0.- dx,0.6*maxY-dx), (0.+ dx,0.6*maxY-dx),(0.+dx,maxY+dx),(0.-dx,maxY+dx)])
    fault3 = fn.shape.Polygon( faultShape3 )
   
    #Case1
    
    faultShape1  = np.array([ (0.1- dx,0.-dx), (0.1+ dx,0.-dx),(0.1+dx,0.33*maxY+dx),(0.1-dx,0.33*maxY+dx)])
    fault1 = fn.shape.Polygon( faultShape1 )

    faultShape2  = np.array([ (0. - dx,0.33*maxY), (0.+dx,0.33*maxY),(0.+dx,0.66*maxY),(0.-dx,0.66*maxY)])
    fault2 = fn.shape.Polygon( faultShape2 )
    
    faultShape3  = np.array([ (-0.1- dx,0.66*maxY-dx), (-0.1+ dx,0.66*maxY-dx),(-0.1+dx,maxY+dx),(-0.1-dx,maxY+dx)])
    fault3 = fn.shape.Polygon( faultShape3 )
    
    #Case2
    
    faultShape1  = np.array([ (0.1- dx,0.-dx), (0.1+ dx,0.-dx),(-0.+dx,0.33*maxY+dx),(-0. -dx,0.33*maxY+dx)])
    fault1 = fn.shape.Polygon( faultShape1 )

    faultShape2  = np.array([ (-0.1 - dx,0.33*maxY), (-0.1+dx,0.33*maxY),(-0.2+dx,0.66*maxY),(-0.2-dx,0.66*maxY)])
    fault2 = fn.shape.Polygon( faultShape2 )
    
    faultShape3  = np.array([ (-0.3- dx,0.66*maxY-dx), (-0.3+ dx,0.66*maxY-dx),(-0.4+dx,maxY+dx),(-0.4-dx,maxY+dx)])
    fault3 = fn.shape.Polygon( faultShape3 )
    ...
    #case2 / 3   
    faultShape1  = np.array([ (0.2- dx,0.-dx), (0.2+ dx,0.-dx),(0.1+dx,0.31*maxY+dx),(0.1 -dx,0.31*maxY+dx)])
    fault1 = fn.shape.Polygon( faultShape1 )

    faultShape2  = np.array([ (-0.1 - dx,0.34*maxY), (-0.1+dx,0.34*maxY),(-0.+dx,0.65*maxY),(-0.-dx,0.65*maxY)])
    fault2 = fn.shape.Polygon( faultShape2 )
    
    faultShape3  = np.array([ (-0.2- dx,0.68*maxY-dx), (-0.2+ dx,0.68*maxY-dx),(-0.3+dx,maxY+dx),(-0.3-dx,maxY+dx)])
    fault3 = fn.shape.Polygon( faultShape3 )
    
    #Case5
    faultShape1  = np.array([ (0.2- dx,0.-dx), (0.2+ dx,0.-dx),(0.2+dx,0.25*maxY+dx),(0.2-dx,0.25*maxY+dx)])
    fault1 = fn.shape.Polygon( faultShape1 )

    faultShape2  = np.array([ (0.1 - dx,0.25*maxY), (0.1+dx,0.25*maxY),(0.1+dx,0.5*maxY),(0.1-dx,0.5*maxY)])
    fault2 = fn.shape.Polygon( faultShape2 )
    
    faultShape3  = np.array([ (0. - dx,0.5*maxY), (0.+dx,0.5*maxY),(0.+dx,0.75*maxY),(0.-dx,0.75*maxY)])
    fault3 = fn.shape.Polygon( faultShape3 )
    
    faultShape4  = np.array([ (-0.1 - dx,0.75*maxY), (-0.1+dx,0.75*maxY),(-0.1+dx,maxY),(-0.1-dx,maxY)])
    fault4 = fn.shape.Polygon( faultShape4 )
    

    
    #Case4
    faultShape1  = np.array([ (0.2- dx,0.-dx), (0.2+ dx,0.-dx),(0.2+dx,0.5*maxY+dx),(0.2-dx,0.5*maxY+dx)])
    fault1 = fn.shape.Polygon( faultShape1 )

    faultShape2  = np.array([ (-0.2 - dx,0.5*maxY), (-0.2+dx,0.5*maxY),(-0.2+dx,maxY),(-0.2-dx,maxY)])
    fault2 = fn.shape.Polygon( faultShape2 )
    
    #Case3
    faultShape1  = np.array([ (0.4- dx,0.-dx), (0.4+ dx,0.-dx),(-0.+dx,0.33*maxY+dx),(-0. -dx,0.33*maxY+dx)])
    fault1 = fn.shape.Polygon( faultShape1 )

    faultShape2  = np.array([ (-0.2 - dx,0.4*maxY), (-0.2+dx,0.4*maxY),(0.2+dx,0.6*maxY),(0.2-dx,0.6*maxY)])
    fault2 = fn.shape.Polygon( faultShape2 )
    
    faultShape3  = np.array([ (-0.2- dx,0.86*maxY-dx), (-0.2+ dx,0.86*maxY-dx),(0.1+dx,maxY+dx),(0.1-dx,maxY+dx)])
    fault3 = fn.shape.Polygon( faultShape3 ) 
    
    dx = 2 * 0.5*maxY/resY
    faultShape1  = np.array([ (0.- dx,maxY/2-dx), (0.0+dx,maxY/2-dx),(0.0+dx,maxY/2+dx),(0.0+dx,maxY/2-dx)])
    fault1 = fn.shape.Polygon( faultShape1 )
    '''
    dx = maxY/resY
    #Case1

    #faultShape1  = np.array([ (0.- dx,0.), (0.0+dx,0.),(0.0+dx,maxY),(0.0-dx,maxY)])
    #fault1 = fn.shape.Polygon( faultShape1 )    

    faultShape1  = np.array([ (0.- dx,0.5*maxY), (0.+ dx,0.5*maxY),(0. +dx,0.5*maxY+dx),(0. -dx,0.5*maxY+dx)])
    fault1 = fn.shape.Polygon( faultShape1 )
    '''
    faultShape2  = np.array([ (-0.1 - dx,0.37*maxY), (-0.1+dx,0.37*maxY),(0.1+dx,0.63*maxY),(0.1-dx,0.63*maxY)])
    fault2 = fn.shape.Polygon( faultShape2 )
    
    faultShape3  = np.array([ (-0.2- dx,0.68*maxY-dx), (-0.2+ dx,0.68*maxY-dx),(-0.3+dx,maxY+dx),(-0.3-dx,maxY+dx)])
    fault3 = fn.shape.Polygon( faultShape3 )
    '''
    #faultShape2  = np.array([ (-0.9- dx,0.), (-0.9+dx,0.),(-0.9+dx,maxY/4+dx),(-0.9-dx,maxY/4+dx)])
    #fault2 = fn.shape.Polygon( faultShape2 )
    # initialise friction to be friction0
    # stickAirYcoord = boxWidth - stickyAirthick + nd(3.* u.kilometer)
    #plasticStrain.data[:] = 0.
    for index in range( len(swarm.particleCoordinates.data) ):
        ...
        #if (fault2.evaluate(tuple(coord))):
        #    plasticStrain.data[index] = 2.
        #if (fault3.evaluate(tuple(coord))):
        #    plasticStrain.data[index] = 2.
            
    coord = fn.input()
    conditions = [((coord[1] < minY+bufferLayer2) &
                   (coord[0] < maxX-bufferLayer3) &
                   (coord[0] > minX+bufferLayer3), materialB),
                  ((coord[1] > maxY-bufferLayer2) &
                   (coord[0] < maxX-bufferLayer3) &
                   (coord[0] > minX+bufferLayer3), materialB),
                  (       True ,           materialA ) ]
...
C0 = nd(2.0e7* u.pascal)
F0 = 0.6
C1 = nd(2.0e6* u.pascal)
F1 = 0.1
...
viscosityA = nd(1e23 * u.pascal * u.second) 
#viscosityA = upcrustViscosity
mu = nd(1e10 * u.pascal)                  # elastic modulus
min_viscosity = nd(1e18*u.pascal*u.second)   

if (Elasticity == True):
    mappingDictViscosity1   = { materialA: viscosityA,
                                materialB: nd(1e25*u.pascal*u.second)}
    ...
    dt_e    = nd(9000*u.year)                        # elastic time step
...
previousStress_xx = np.zeros(nsteps)
while step<nsteps:
    # Obtain V,P and remove null-space / drift in pressure

    solver.solve( nonLinearIterate=True,  nonLinearTolerance=1e-3, nonLinearMaxIterations=20)
    '''
    (area,) = surfaceArea.evaluate()
    (p0,) = surfacePressureIntegral.evaluate() 
    pressureField.data[:] -= p0 / area
    ...
    previousStress_xx[step] = previousStress[0].evaluate(swarm)[0]
    ...
    '''
    for index in mesh.specialSets["MinI_VertexSet"]:
        velocityField.data[index] = [-newmeshV, 0.]
    for index in mesh.specialSets["MaxI_VertexSet"]:
        velocityField.data[index] = [ newmeshV, 0.]
    '''
np.save(outputPath+'stressXX',previousStress_xx)
np.save(outputPath+'tTracer',tTracer)
